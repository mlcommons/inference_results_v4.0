...
def convert_torch_to_onnx(pytorch_model_dir, output_dir, model_name):
    ...
    if model_name == 'text_encoder':
        ...
        onnx_name = 'vae'
    elif model_name == 'unet' or model_name == 'shallowunet':
        logging.fatal(f"unet model: {model_name} onnx is directly provided")
    else:
        logging.fatal(f"Unrecognized model: {model_name}")
...
def generate_onnx_sdxl(model_dir, output_dir):
    ...
    # we have our own unet onnx and shallowunet onnx
    models = ['text_encoder', 'text_encoder_2', 'vae']
    for model in models:
