...
class SDXLUNetXLBuilder(SDXLBaseBuilder,
                        ArgDiscarder):
    """SDXL ShallowUNetXL builder class.
    """
...
class SDXLShallowUNetXLBuilder(SDXLBaseBuilder, ArgDiscarder):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__(*args,
                         model=ShallowUNetXL(max_batch_size=batch_size, precision=precision, device='cuda'),
                         model_path=model_path,
                         batch_size=batch_size,
                         **kwargs)
...
class SDXLEngineBuilderOp(Operation, ArgDiscarder):
    ...
    def __init__(self,
                 *args,
                 ...
                 open_model_path: str = "build/open-models/SDXL/",
                 **kwargs):
        ...
        # onnx.load() has issue reading weights separately under the same path as model file if input path is PosixPath, use str instead
        if precision == 'int8':
            unetxl_path = open_model_path + "onnx_models/unetxl/deep.int8/unet.onnx"
            shallowunetxl_path = open_model_path + "onnx_models/unetxl/shallow.int8/unet.onnx"
        elif precision == 'fp16':
            unetxl_path = open_model_path + "onnx_models/unetxl/deep/unet.onnx"
            shallowunetxl_path = open_model_path + "onnx_models/unetxl/shallow/unet.onnx"
        else:
        ...
        self.shallowunetxl_builder = SDXLShallowUNetXLBuilder(*args, precision=precision, model_path=shallowunetxl_path, **kwargs)
    ...
    def run(self, scratch_space, dependency_outputs):
        ...
        logging.info(f"Building ShallowUNetXL from {self.shallowunetxl_builder.model_path}")
        shallowunetxl_network = self.shallowunetxl_builder.create_network(use_native_instance_norm=False)
        shallowunetxl_engine_name = self.shallowunetxl_builder.engine_name(self.shallowunetxl_builder.device_type,
                                                                           self.shallowunetxl_builder.batch_size,
                                                                           self.shallowunetxl_builder.precision,
                                                                           tag=self.config_ver)
        self.shallowunetxl_builder.build_engine(network=shallowunetxl_network,
                                                builder_config=self.shallowunetxl_builder.create_builder_config(),
                                                batch_size=None,  # determined by shallowunetxl_builder.batch_size
                                                save_to=engine_dir / shallowunetxl_engine_name)
