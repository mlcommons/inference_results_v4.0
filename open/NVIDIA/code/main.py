...
def populate_config_registry(benchmarks: List[Benchmark], scenarios: List[Scenario]):
    ...
    for benchmark in benchmarks:
        for scenario in scenarios:
            requested.append(f"{benchmark}.{scenario}")
            if not ConfigRegistry.load_configs(benchmark, scenario):
                not_found.append(f"{benchmark}.{scenario}")
    if len(not_found) > 0:
        logging.warning(f"Requested benchmark and scenario: {requested}, but some of them do not have config: {not_found}")
    ...
    for benchmark in benchmarks:
        for scenario in scenarios:
            ...
            default_workload = WorkloadSetting(
                ...
                model_opt=ModelOpt.get_match(main_args["model_opt"]))
