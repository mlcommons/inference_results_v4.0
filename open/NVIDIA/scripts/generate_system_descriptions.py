...
H100_NVL = Machine(
    ...
    accelerator_short_name="H100-NVL-94GB",
    mig_short_name="",
    ...
    sw_notes="",
    system_id_prefix="",
    system_name_prefix="SYS-521GE-TNRT",
    host_memory_configuration="32x 64GB M321R8GA0BB0-CQKZJ",
    host_networking="Gig Ethernet; Data bandwidth for GPU-PCIe: 504 GB/s",
    host_network_card_count="2x x710 10Gbe; 2x ConnectX-4",
    host_networking_topology="Ethernet on switching network",
    accelerator_host_interconnect="PCIe Gen4 x16",
    accelerator_interconnect="18x 4th Gen NVLink, 600GB/s",
    cooling="Air-cooled",
    system_type_detail="N/A",
    power_supply_details="",
    power_supply_quantity_and_rating_watts="",
...
class System():
    def __init__(self, machine, division, system_type, gpu_count=1, mig_count=0, is_triton=False, is_soc=False, is_maxq=False, model_opt=ModelOpt.NoOpt, status=None):
        self.attr = {
            ...
            "system_name": self._get_system_name(machine, division, gpu_count, mig_count, is_triton, is_soc, is_maxq),
            "number_of_nodes": 1,
    ...
    def _get_system_id(self, machine, division, gpu_count, mig_count, is_triton, is_soc, is_maxq, model_opt: ModelOpt):
        return "".join([
            ...
            f"_{model_opt.upper_id()}",
        ])

    def _get_system_name(self, machine, division, gpu_count, mig_count, is_triton, is_soc, is_maxq):
        system_details = []
        if not is_soc:
...
submission_systems = [
    ...
    System(H100_NVL, Division.CLOSED, SystemType.DATACENTER, 8, 0, False, False),  # H100_NVL
    ...
    System(DGX_H200_CTS_AC, Division.CLOSED, SystemType.DATACENTER, 1, 0, False, False),  # H200-CTS-SXM-80GBx1
    System(DGX_H200_CTS_AC, Division.CLOSED, SystemType.DATACENTER, 8, 0, False, False),  # H200-CTS-SXM-80GBx8
    System(GH200_GraceHopper_Superchip_Starship, Division.CLOSED, SystemType.DATACENTER, 1, 0, False, False),  # Starship G+H CG1
    System(GH200_GraceHopper_Superchip_MGX, Division.CLOSED, SystemType.DATACENTER, 1, 0, False, False),  # Lego MGX G+H CG1
...
open_submission_systems = [
    # Datacenter submissions
    System(DGX_H100, Division.OPEN, SystemType.DATACENTER, gpu_count=8, model_opt=ModelOpt.Sparse, status=Status.AVAILABLE),  # H100-SXM-80GBx8
    System(DGX_H100, Division.OPEN, SystemType.DATACENTER, gpu_count=1, model_opt=ModelOpt.Pruned, status=Status.AVAILABLE),  # H100-SXM-80GBx1
    System(DGX_H100, Division.OPEN, SystemType.DATACENTER, gpu_count=1, model_opt=ModelOpt.DeepCache, status=Status.AVAILABLE),  # H100-SXM-80GBx1
    System(DGX_H100, Division.OPEN, SystemType.DATACENTER, gpu_count=1, model_opt=ModelOpt.DeepCachePruned, status=Status.AVAILABLE),  # H100-SXM-80GBx1
    # System(DGX_H100, Division.OPEN, SystemType.DATACENTER, 2, 0, False, False),  # H100-SXM-80GBx2
    # System(DGX_H200_Full_AC, Division.OPEN, SystemType.DATACENTER, 8, 0, False, False),  # H200-Full-SXM-80GBx8
