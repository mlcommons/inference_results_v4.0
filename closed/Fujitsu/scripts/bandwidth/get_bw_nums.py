...
def gpu_pcie_bandwidth_calc():
    nvsmi_proc = sp.run(["nvidia-smi", "--query-gpu", "index,pci.bus_id,pcie.link.gen.gpucurrent,pcie.link.gen.gpumax,pcie.link.gen.hostmax,pcie.link.width.current,pcie.link.width.max", "--format=csv,noheader"], capture_output=True)
