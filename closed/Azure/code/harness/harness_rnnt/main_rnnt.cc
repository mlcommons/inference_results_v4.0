...
public:
    Stream(const int deviceId, std::shared_ptr<qsl::SampleLibrary> qsl, std::shared_ptr<WarmupSampleLibrary> wqsl,
        ...
        const std::string& engineDir, const size_t batchSize, PerfTrPrinter &printer);
...
)
{
    ...
    auto numResponses = mDecoderState.send_responses_and_clear(completion_function);
    if (numResponses > 0) {
        PERF_TR_EVENT(perfTr::kDEQ, perfTr::kBS, numResponses, mPrinter);
    }
}
...
void Stream::processBatchesFrontEnd()
{
    ...
    while (!mDone)
    {
        ...
        nvtxRangeId_t nvtxRunEnc;
        NVTX_GLOBAL_START_WITH_PAYLOAD(nvtxRunEnc, "ENC:run", nvtx::COLOR_BLUE_0, actualBatchSize[p]);
        if (avail_samples)
        {
            frontEndBatch(p);
        }
...
void Stream::processBatchesBackEnd()
{
    ...
    while (!mDone)
    {
        ...
        // run decoder portion
        nvtxRangeId_t nvtxRunDec;
        NVTX_GLOBAL_START_WITH_PAYLOAD(nvtxRunDec, "DEC: run", nvtx::COLOR_GREEN_0, actualBatchSize[p]);
        ...
        for (size_t streamIdx = 0; streamIdx < streamsPerGpu; streamIdx++)
        {
            ...
            if (FLAGS_enable_audio_processing)
            {
                mStreams.emplace_back(new Stream(deviceId, mQsl, mWqsl, mWorkQueues[workQueueIdx],
                    mAudioBufManagers[numDevices], enginePath, maxBatchSize, mPrinter));
            }
            else
            {
                mStreams.emplace_back(
                    new Stream(deviceId, mQsl, mWqsl, mWorkQueues[workQueueIdx], nullptr, enginePath, maxBatchSize, mPrinter));
            }
        }
        ++numDevices;
        ++workQueueCounter;
...
void RNNTServer::IssueQuery(const std::vector<mlperf::QuerySample>& samples)
{
    ...
    if (mUseBatchSort)
    {
        ...
        PERF_TR_EVENT(perfTr::kENQ, perfTr::kBS, sortedSamples.size(), mPrinter);
        mWorkQueues[threadIdx]->insertItems(sortedSamples);
    }
    else
    {
        PERF_TR_EVENT(perfTr::kENQ, perfTr::kBS, samples.size(), mPrinter);
