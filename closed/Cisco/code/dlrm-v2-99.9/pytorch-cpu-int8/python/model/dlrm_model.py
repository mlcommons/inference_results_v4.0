...
class SparseArch(nn.Module):
    ...
    def __init__(self, embedding_bag_collection: MergedEmbWithCat) -> None:
        super().__init__()
        self.embedding_bag_collection: MergedEmbWithCat = embedding_bag_collection
        self._sparse_feature_names: List[str] = [
            name for name, param in embedding_bag_collection.named_parameters()]

    def forward(self, index, dense) -> torch.Tensor:
        """
        Args:
        ...
        """
        return self.embedding_bag_collection(index, dense)
    # return y
...
class DLRMMLPerf(nn.Module):
    def __init__(
        ...
        self.sparse_arch: SparseArch = SparseArch(
            MergedEmbWithCat(embedding_dim,
                             num_embeddings_pool))
    ...
    def forward(self, densex, index) -> torch.Tensor():
        embedded_dense = self.dense_arch(densex)
        combined_values = self.sparse_arch(index, embedded_dense)
