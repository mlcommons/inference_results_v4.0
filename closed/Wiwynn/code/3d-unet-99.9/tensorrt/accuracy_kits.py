...
def load_loadgen_log(log_file: str,
    ...
    """
    predictions = {}  # store qsl_idx: line_num
    line_offset = []
    line_num, offset = 0, 0
        ...
        for line in f:
            ...
            if 'qsl_idx' in line:
                if line.strip().endswith(','):
                    prediction = json.loads(line.strip().rstrip(','))
                else:
                    prediction = json.loads(line)
                predictions[prediction['qsl_idx']] = line_num
            offset += len(line)
            line_num += 1

    assert len(predictions.keys()) == len([k for k in aux.keys() if k in file_list]),\
        "Number of predictions does not match number of samples in validation set!"

    results = dict()
    with open(log_file) as f:
        for qsl_idx, line_num in predictions.items():
            case = file_list[qsl_idx]
            f.seek(line_offset[line_num])
            line = f.readline()
            if line.strip().endswith(','):
                prediction = json.loads(line.strip().rstrip(','))
            else:
