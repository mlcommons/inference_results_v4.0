...
SUPPORTED_MODELS=("resnet50" "retinanet" "rnnt" "3d-unet" "bert" "gpt-j" "dlrm_2" "all")
WORK_DIR=`pwd`
...
function download_dlrm2() {
    ...
    local model_dir=${data_dir}/model/bf16
    mkdir -p ${model_dir}
...
function download_gptj() {
    ...
    # if [ "1${dtype}" == '1int4' ]; then
    #     echo "Running caliberation for int4..."
    #     GPTJ_CALIBRATION_INT4_DIR=${GPTJ_CALIBRATION_DIR}/INT4
    #     pushd ${GPTJ_CALIBRATION_INT4_DIR}
    #     source ${conda_path}/etc/profile.d/conda.sh
    #     conda env remove -n gpt-j-calibration-int4-env
    #     conda env create -f quantization-env.yaml
    #     conda activate gpt-j-calibration-int4-env
    #     conda info
    #     export WORKLOAD_DATA=${dataset_dir}
    #     #######################################
    #     # copy from setup_env.sh
