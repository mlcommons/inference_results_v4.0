...
class CriteoDay23Dataset:
    ...
    def __init__(self, data_dir: os.PathLike, mode: str = "full", precision="fp32"):
        ...
        self.use_sparse_concat = self.sparse_concat_path.exists()
    ...
    def load_data(self):
        ...
        if not self.use_sparse_concat:
            ...
            sparse_inputs = np.vstack(sparse_inputs)
        else:
    ...
    def generate_int8_dense(self, dst_dir: os.PathLike):
        new_dense_path = dst_dir / "day_23_dense.npy"
        transformed_dense = np.log(np.maximum(0, self.dense) + 1)
        dense_scale = "3de5d364"
        SCALE_FACTOR = 127 * struct.unpack('!f', bytes.fromhex(dense_scale))[0]
        int8_factor = 127.0 / SCALE_FACTOR
        int8_dense = np.minimum(127.0, np.maximum(-128.0, int8_factor * transformed_dense))
        int8_dense = int8_dense.astype(np.int8)
        np.save(new_dense_path, int8_dense)
    ...
    def get_batch(self, num_samples=None, indices=None):
        ...
        if not self.use_sparse_concat:
            sparse_batch = self.sparse[:, indices]
        else:
            sparse_batch = self.sparse[indices]
