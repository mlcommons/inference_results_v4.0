...
def main():
    ...
    if (args.quantize or args.measure) and args.hpus==1:
        # import quantization package and load quantization configuration
        import habana_quantization_toolkit
        config_obj = habana_quantization_toolkit._quant_common.quant_config.Fp8cfg()
    ...
    if args.hpus == 1:
        ...
        if args.quantize or args.measure:
            ...
            habana_quantization_toolkit.prep_model(backend.pipe.unet)
            if args.quantize:
                config_obj.update_config(config_no=2)
                habana_quantization_toolkit.prep_model(backend.pipe.unet_bf16)
        if args.hpu_graph and torch_hpu.is_available():
