...
int MemoryManager::init_memory_slots () {
  for (int i = 0; i < MAX_MEM_REGIONS; i++) {
    ...
    allocate_slot(mem_cfg_full_mode[i].idx);
...
void MemoryManager::allocate_slot (int idx) {
  ...
  if (!mem_cfg_full_mode[idx].is_reserved) {
    ...
    std::cout << "MLPerf::[memory] Allocating memory for slot_id " << mem_cfg_full_mode[idx].idx << " size " << mem_sz << "\n";
    ...
    //std::cout << "phys addr " << std::hex << simaai_memory_get_phys(m_handle) << "\n";
    int slot_id = mem_cfg_full_mode[idx].idx;
    add_buffer_id_mapping (slot_id, simaai_memory_get_id(m_handle));
  }
};
#else // MLART_USE_SIMA_MEM_LIB
void MemoryManager::allocate_slot (int idx) {
  if (idx < 0)
    throw std::runtime_error("Undefined slot id");

  printf("slot 0x%x - map slot name %s, target id %d, max size %ld, rsvd %d\n",
          idx, mem_cfg_full_mode[idx].slot_name, mem_cfg_full_mode[idx].target_id,
          mem_cfg_full_mode[idx].max_size, mem_cfg_full_mode[idx].is_reserved);
  ...
  if (!mem_cfg_full_mode[idx].is_reserved) {
    std::cout << "slot " << idx << " using phys addr " << std::hex << simaai_memory_get_phys(m_handle) << "\n";
    int slot_id = mem_cfg_full_mode[idx].idx;
    add_buffer_id_mapping (slot_id, simaai_memory_get_id(m_handle));
  } else {
    std::cout << "slot " << idx << " not using phys addr " << std::hex << simaai_memory_get_phys(m_handle) << "\n";
  }
};
#endif // MLART_USE_SIMA_MEM_LIB
