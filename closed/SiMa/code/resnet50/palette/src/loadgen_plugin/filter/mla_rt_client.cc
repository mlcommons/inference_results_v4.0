 ...
 */
int mla_rt_client_run_model (simamm_buffer_id_t in_buf_id,
			     ...
			     uint32_t out_buf_offset)
{
  ...
  if (is_prepared.load() == false) {
    in_addr = buffer_id_to_paddr(in_buf_id);
    out_addr = buffer_id_to_paddr(out_buf_id) + out_buf_offset;
    is_prepared.store(true);
  }
                                   ...
                                   size_t out_sz)
{
  if (is_prepared.load() == false) {
    ...
    out_addr_list = new uint64_t[batch_size];
...
{
  std::vector<std::string> arguments = {"libMLArt.so", "--ocm", "max"};
  ...
  simaailog_init(NULL);
  handle = (uint64_t) mla_get_handle_argv(argv.size() - 1, argv.data());
