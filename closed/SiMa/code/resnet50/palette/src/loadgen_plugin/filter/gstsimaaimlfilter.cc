...
struct _GstMlFilterPrivate
{
  ...
  gchar *output_path; ///< output path where mlperf_log* files would be stored
    ...
    const GValue *value, GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_OUTPUT_PATH:
      self->priv->output_path = g_strdup (g_value_get_string(value));
      break;
    case PROP_SCENARIO:
...
gst_ml_filter_get_loadgen_config (GstMlFilter * self, simaai::mlperf_wrapper::LoadgenConfig & cfg) {
  ...
  if (self->priv->output_path != NULL)
    cfg.output_path = self->priv->output_path;
  else
    cfg.output_path = ".";
}
...
gst_ml_filter_start (GstBaseTransform * trans)
{
  ...
  /// Hackery for batch_one to check if the static allocation has an impact.
#ifdef SIMAAI_SUPPORT_MLA_RELOCS
  if (self->priv->in_tensor_dim.dim[0] == 1) {
    size_t in_sz = gst_ml_filter_get_tensor_size(self, IN_TENSOR_TYPE);
    self->priv->in_mem = simaai_target_mem_alloc_flags (SIMAAI_MEM_TARGET_M4,
                                                        in_sz,
                                                        &self->priv->buf_id,
                                                        SIMAAI_MEM_FLAG_DEFAULT);
  }
#endif
  ...
  if (self->priv->out_mem == NULL) {
    ...
    g_assert(self->priv->out_mem == NULL);
  }

  GST_DEBUG_OBJECT(self, "IN: Buffer id = %ld, Out: Buffer id = %ld", self->priv->buf_id, self->priv->out_buf_id);
			...
			GstBuffer *outbuf)
{
  ...
  if (map.data == NULL) {
    GST_ERROR("Unable to map in_mem handle\n");
  ...
  uint64_t mla_latency_min = 0;
  uint64_t mla_latency_max = 0;
  double mla_latency_avg, mla_latency_avg0, mla_latency_var, mla_latency_var0 = 0;
  int mla_count = 0;
#endif
  ...
  while (true) {
    ...
    if (data_len <= 0) {
      GST_ERROR_OBJECT(self, "data is invalid, size returned : [%ld]", self->priv->bytes_available);
      return GST_FLOW_ERROR;
    }
...
#ifdef SIMAAI_SUPPORT_MLA_RELOCS
    if (batch_size == 1) {
      off_t page_base = (phys_addr_arr[0] / pagesize) * pagesize;
      off_t page_offset = phys_addr_arr[0] - page_base;

      uint8_t * mem = (uint8_t *)mmap(NULL, page_offset + in_img_sz, PROT_READ , MAP_PRIVATE, fd, page_base);
      if (mem == MAP_FAILED) {
        GST_ERROR("Unable to map 0x%lx, size= %ld, error=%s\n", phys_addr_arr[0], in_img_sz, strerror(errno));
        return GST_FLOW_ERROR;
      }

      memcpy(&map.data[0], &mem[page_offset], in_img_sz);
      munmap(mem, page_offset + in_img_sz);
...
#endif
#ifdef ML_FILTER_EVAL_PERF
      struct timeval mla_start, mla_end;
      uint64_t mla_seconds, mla_micros;
      gettimeofday(&mla_start, NULL);
#endif
...
#ifdef ML_FILTER_EVAL_PERF
      gettimeofday(&mla_end, NULL);

      // Account for usec rollover which causes negative values
      mla_seconds = (mla_end.tv_sec - mla_end.tv_sec);
      mla_micros = mla_seconds * 1000000LL;
      mla_micros += mla_end.tv_usec - mla_start.tv_usec;
      if (mla_micros < 0) {
        mla_micros += 1000000LL;
        --mla_micros;
      }
      // printf("[%d]:[%s] runtime %lld secs, %lld micros\n", __LINE__, __func__, mla_seconds , mla_micros);

      if ((mla_micros < mla_latency_min) || (mla_count == 1))
        mla_latency_min = mla_micros;
      if (mla_micros > mla_latency_max)
        mla_latency_max = mla_micros;
      if (mla_count == 1) {
        mla_latency_avg = mla_latency_avg0 = mla_micros;
        mla_latency_var = mla_latency_var0 = 0;
      } else {
        double u = mla_latency_avg, u0 = mla_latency_avg0;
        double v = mla_latency_var, v0 = mla_latency_var0;
        mla_latency_avg = u0 + ((mla_micros - u0) / mla_count);
        mla_latency_var = v0 + ((mla_micros - u0) * (mla_micros - u));
        mla_latency_avg0 = u;
        mla_latency_var0 = v;
        //printf("Run %d, Min %lld, Avg %.2f, Max %lld\n", mla_count, mla_latency_min, mla_latency_avg, mla_latency_max);
      }
#endif // ML_FILTER_EVAL_PERF
#ifdef SIMAAI_SUPPORT_MLA_RELOCS
      }
#endif
...
static int gst_ml_filter_run_topk (GstMlFilter * self, const int8_t * data) {
...
#ifdef NO_ARGMAX_MODEL
  int8_t * classes = (int8_t *) data;
#else
  uint8_t * classes = (uint8_t *) data;
#endif
  ...
  for (size_t img = 0; img < batch_size; img++) {
...
#ifdef NEON_SIMD
    int8_t best_val = 0;
    size_t best_i = argmax_s8(&classes[img * 1008], MLA_VALID_NDIM, &best_val);
#else // ARGMAX on APU
    ...
    best_i = best_indexes[best_indexes.size() - 1] - 1;
#endif // NEON_SIMD
#else  // MODEL WITH NO_SOFTMAX
    uint16_t best_i = (classes[ndim * img + 16] << 8 | classes[ndim * img]);
#endif
    GST_DEBUG_OBJECT(self, "seq_id = %ld, image_id1 = %ld, class_id = %d",
		     self->priv->frame_id,
  ...
  if (!self->silent) {
    ...
    seconds = (end.tv_sec - start.tv_sec);
    micros = ((seconds * 1000000) + end.tv_usec) - (start.tv_usec);
    fprintf(stderr,"[%d]:[%s] runtime %ld secs, %ld micros\n", __LINE__, __func__, seconds , micros);
  }
  ...
  g_object_class_install_property (gobject_class, PROP_TOY_MODE,
                                   g_param_spec_boolean ("toy-mode", "Enable Toy Mode",
                                                         ...
                                                         false, GParamFlags(G_PARAM_READWRITE |
                                                                            GST_PARAM_MUTABLE_PLAYING |
                                                                            G_PARAM_STATIC_STRINGS)));
  // Use "-" in the gstreamer property name, "_" does not work
  g_object_class_install_property (gobject_class, PROP_OUTPUT_PATH,
                                   g_param_spec_string ("output-path", "Store output files",
                                                         "Path to store mlperf_log* output files",
...
gst_ml_filter_init (GstMlFilter *self)
{
  ...
  self->priv->in_mem = 0;
  self->priv->out_mem = 0;
  self->priv->output_path = NULL;
  self->priv->toy_mode = false;
