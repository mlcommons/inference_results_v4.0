...
class LLAMA2EngineBuilderOp(TRTBuilder,
    ...
    def __init__(self,
                 ...
                 # TODO: fix this.
                 fp8_quant_model_path: str = "build/models/Llama2/fp8-quantized-ammo/llama2-70b-tp2pp1-fp8-quantized-01312024",
        ...
        if self.use_fp8:
            if not self.fp8_quant_model_path.exists():
                raise FileNotFoundError(f"Could not locate Llama2 fp8 quantized checkpoint model path: ({self.fp8_quant_model_path})")
                # TODO If the default model does not exist, build a quantized model locally
                # self.need_quantization = True
        else:
            raise NotImplementedError(f"Only fp8 supported as of now. Set precision to FP16 and use_fp8 to True.")
    ...
    def build_engine(self,
        ...
        flags = ["--use_gpt_attention_plugin=float16",
