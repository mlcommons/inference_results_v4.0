    ...
    , mTotalSampleConsumed(0)
{
    ...
    mRecorder = std::make_unique<Recorder>(true);
...
LLMServerLeader::LLMServerLeader(std::string const& name, std::shared_ptr<qsl::SampleLibrary> qsl,
    ...
    int32_t responderLocalRank, bool verboseNVTX)
    ...
    , mRecvBuffer(useMPI ? kMAX_TRANSACTIONS_IN_FLIGHT : 0,
...
LLMServerLoadGen::LLMServerLoadGen(std::string const& name, std::shared_ptr<qsl::SampleLibrary> qsl,
    ...
    std::vector<std::shared_ptr<MPI_Comm>> firstTokComms, int32_t loadgenRank, int32_t distrPol, bool verboseNVTX)
    : mName(name)
    , mQsl(qsl)
    , mLLMConfig(LLMConfig)
    ...
    , mServerStopWork(false)
    , mRecvBuffer(kMAX_TRANSACTIONS_IN_FLIGHT,
          excludeInputInOutput ? kMAX_BUFFER_BYTESIZE_PER_TRANSACTION : kMAX_BUFFER_BYTESIZE_PER_TRANSACTION_WITH_INPUT,
          name)
    , mFirstTokBuffer(kMAX_TRANSACTIONS_IN_FLIGHT, 1, "first token comm buffer")
    , mLastUsedQueueIdx(0)
