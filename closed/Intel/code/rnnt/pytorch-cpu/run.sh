...
if [[ ${STAGE} -le 4 ]]; then
  ...
  if [ "${SETUP_ONLY}" == "yes" ];then exit;fi
fi

if [[ ${STAGE} -le 5 ]]; then
  echo '==> Run RNN-T Offline accuracy'
  SCENARIO=Offline BS=256 LEN=2 ACCURACY=true ./launch_sut.sh
  sleep 5
  echo '==> Run RNN-T Offline benchmark'
  SCENARIO=Offline BS=256 LEN=2 WARMUP=3 ./launch_sut.sh
  sleep 5
  echo '==> Run RNN-T Server accuracy'
  SCENARIO=Server PRO_BS=4 BS=128 LEN=8 RESPONSE=9 QOS=233500 ACCURACY=true ./launch_sut.sh
  sleep 5
  echo '==> Run RNN-T Server benchmark'
  SCENARIO=Server PRO_BS=4 BS=128 LEN=8 RESPONSE=9 QOS=233500 WARMUP=3 ./launch_sut.sh
  wait
fi
