...
class Coco(dataset.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.captions_df = pd.read_csv(f"{data_path}/captions/captions_source.tsv", sep="\t")
        print("Length of captions_df : ",len(self.captions_df))
        self.captions_df = self.captions_df.iloc[:total_sample_count]
        print("Length of captions_df after total_sample_count : ",len(self.captions_df))
        if "warmup" not in data_path:
            self.captions_df[
            ["id", "image_id", "caption", "height", "width", "file_name", "coco_url"]
        ].to_csv(f"{data_path}/captions/captions.tsv", sep="\t", index=False)
        ...
        if "warmup" not in data_path:
            ...
            self.latents = torch.load(f"{data_path}/../latents/latents.pt").to(latent_dtype)
                    
        self.count = len(self.captions_df)
        self.perf_count = len(self.captions_df)

        for sample in range(0, len(self.captions_df)):
            self.items_inmemory[sample] = self.get_item(sample)
        self.last_loaded = time.time()

    def load_latent(self, device):
        for sample in range(0, len(self.captions_df)):
            self.items_inmemory[sample]["latents"].to(device)
        #self.latents.to(device)
