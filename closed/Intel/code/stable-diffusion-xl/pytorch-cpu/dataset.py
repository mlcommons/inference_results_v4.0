...
class Dataset(object):
    ...
    def get_samples(self, id_list):
        ...
        latents_list = []
        for id in id_list:
            input_tokens_list.append(self.items_inmemory[id]["input_tokens"].input_ids)
            input_tokens_2_list.append(self.items_inmemory[id]["input_tokens_2"].input_ids)
            latents_list.append(self.items_inmemory[id]["latents"])
        input_tokens = torch.cat(input_tokens_list)
        input_tokens_2 = torch.cat(input_tokens_2_list)
        latents = torch.cat(latents_list)

        return input_tokens, input_tokens_2, latents
