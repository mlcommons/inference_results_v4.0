...
class Instance(mp.Process):
    ...
    def do_warmup(self): # TODO: Fix the samples used??
        ...
        for i in range(5):
            input_tokens, input_tokens_2, latents = self.dataset.get_samples([i for i in range(self.batch_size)])
    ...
    def handleTasks(self, i, task_queue, result_queue, pid, start_core, num_cores):
        ...
        while True:
            ...
            for t in outputs
                ...
                # logger.info(f"query_idx: {query_idx} receipt-finish total time : {t_output_queue-t_receipt:6.2f} || Actual predict latency : {t2-t1:6.2f} || fetch from input queue wait: {t0-t_receipt:6.2f} ")
    ...
    def run(self):
        ...
        for i in range(self.num_workers):
                ...
                worker = mp.Process(target=self.handleTasks, args=(i, self.input_queue, self.output_queue, self.pid, start_core, worker_cores))
