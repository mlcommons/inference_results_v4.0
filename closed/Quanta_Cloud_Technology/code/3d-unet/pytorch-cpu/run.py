...
def main():
    ...
    if len(buckets)==0:
        ...
        total_workers = num_ins * ins_per_consumer 

        i = 0
        # TODO: Consider system config i.e core-socket-numa allocation
        while i < num_ins:
            start_core_idx = i * num_cpus + loadgen_cores
            consumer = Consumer(input_queues, out_queue[i%num_resp_qs], backend_params, dataset_params, lock, cv, init_counter, i, start_core_idx, num_cpus, args, num_workers=ins_per_consumer)
        ...
        for j, cutoff in enumerate(cutoffs):
            ...
            for _ in range(num_ins):
                ...
                consumer = Consumer(input_queues[j], out_queue[i%num_resp_qs], backend_params, dataset_params, lock, cv, init_counter, i, start_core_idx, num_cpus, args, num_workers=ins_per_consumer)
                consumers.append(consumer)
