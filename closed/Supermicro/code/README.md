...
```bash
model={resnet50,retinanet,rnnt,3d-unet,bert,gpt-j,dlrm_2,all} output_dir=<DATA_PATH> conda_path=<CONDA_ROOT_PATH> bash download_data.sh 
```
Parameters:
...
* dtype: optional, int8 by default, data preicision, used by gpt-j only
...
For specific model, e.g. gpt-j int4, use the following command:
```bash
...
| dlrm-v2-99             | CPU         | int8        | `python3 run.py -n dlrm-v2-99 -i pytorch-cpu-int8 -d ${DATA_DIR}/dlrm_2/data_npy -m ${DATA_DIR}/dlrm_2/model -t ${OUTPUT_DIR} -x ${SUFFIX}`             |
...
```
usage: run.py [-h] -n {rnnt,bert-99,3d-unet-99.9,resnet50,retinanet,dlrm-v2-99}
              [-i IMPLEMENTATION] [-y {int8,bf16,fp16,int4,fp32,mix}] -d DATASET -m MODEL_DIR
              -t OUTPUT -x CONTAINER_NAME_SUFFIX [-p] [-a] [-o] [-s] [-c] [-b] [-r]

options:
  -h, --help            show this help message and exit
  -n {rnnt,bert-99,3d-unet-99.9,resnet50,retinanet,dlrm-v2-99}, --model {rnnt,bert-99,3d-unet-99.9,resnet50,retinanet,dlrm-v2-99}
                        Benchmarking model
